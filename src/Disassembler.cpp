...
void Disassembler::disassemble() {
  ...
  // ORI to CCR
    ...
    n_->move(
        size, 
        ident::ident_effective_adr(src_m, src_xn), src_xn, 
    ...
    n_->move_from_sr(ident::ident_effective_adr(m, xn));
  } else if ((op & 0b1111111111000000) 
                == 0b0100010011000000
  ) {
    // MOVE to CCR
    ...
    n_->move_to_ccr(ident::ident_effective_adr(m, xn));
  } else if ((op & 0b1111111111000000) 
                == 0b0100011011000000
  ) {
    // MOVE to SR
    ...
    n_->move_to_sr(ident::ident_effective_adr(m, xn));
  } else if ((op & 0b1111111100000000) 
                == 0b0100000000000000
  ) {
    // NEGX
    ...
    n_->mulu(dn, ident::ident_effective_adr(m, xn));
  } else if ((op & 0b1111000111000000) 
                == 0b1100000111000000
  ) {
    // MULS
    ...
    n_->muls(dn, ident::ident_effective_adr(m, xn));
  } else if ((op & 0b1111000111110000) 
                == 0b1100000100000000
  ) {
    // ABCD
