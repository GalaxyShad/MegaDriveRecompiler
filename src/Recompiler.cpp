...
void Recompiler::ext(Size s, u8 dn) { ///
  // NOT_IMPLEMENTED
  flow_.ctx().writeln(std::format("{} = (0xFFFF{} * ({} >> (sizeof({}) * 8 - "
                                  "1))) - (((sizeof({}) * 8) << 1) - 1) | {};",
                                  Code::dn(dn), s == Size::Byte ? "" : "FFFF",
                                  Code::dn(dn), Code::get_sizeof_size(s),
                                  Code::get_sizeof_size(s), Code::dn(dn)));
}
...
void Recompiler::tst(Size s, AddressingMode m, u8 xn) {
  ...
  post += " ctx->cc.n = (ctx->res < 0); ctx->cc.z = (ctx->res == 0); ctx->cc.v "
          "= 0; ctx->cc.c = 0;";
...
Recompiler::fmt_get_value(const DecodedEffectiveAddress &ea) {
  ...
  case AddressingMode::AddressWithIndex: {
    src =
        Code::deref_adr(ea.size, std::format("{} + {} + {:X}", Code::an(ea.xn),
  ...
  case AddressingMode::PcWithIndex: {
    src = Code::deref_adr(
        ea.size, std::format("{} + {}", ea.pc_with_index, Code::dn(ea.dst_xn)));
    break;
  }
  }
