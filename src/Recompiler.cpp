...
void Recompiler::swap(u8 dn) {///
    flow_.ctx().writeln(
        std::format("{} = (({} & 0xFFFF) << 16) | (({} >> 16) & 0xFFFF);", Code::dn(dn), Code::dn(dn), Code::dn(dn))
    );
}
...
void Recompiler::rod(RotationDirection d, AddressingMode m, u8 xn) {///
    ...
    if (d == RotationDirection::Left) {
        flow_.ctx().writeln(
            pre +
            std::format("{} = ({} << {}) | ({} >> (sizeof({}) * 8 - {}));", src, src, 1, src, Code::get_sizeof_size(Size::Word), 1) +
            post
        );
    } else {
        flow_.ctx().writeln(
...
void Recompiler::rod_rotation(u8 rotation, RotationDirection d, Size s, Rotation m, u8 dn) {///
    ...
    if (d == RotationDirection::Left) {
        flow_.ctx().writeln(
            std::format("{} = ({} << {}) | ({} >> (sizeof({}) * 8 - {}));", Code::dn(dn), Code::dn(dn), count_shift, Code::dn(dn), Code::get_sizeof_size(s), count_shift)
        );
    } else {
        flow_.ctx().writeln(
            std::format("{} = ({} >> {}) | ({} << (sizeof({}) * 8 - {}));", Code::dn(dn), Code::dn(dn), count_shift, Code::dn(dn), Code::get_sizeof_size(s), count_shift)
...
Recompiler::fmt_get_value(const DecodedEffectiveAddress &ea) {
    ...
    switch (ea.mode) {
        ...
        case AddressingMode::AddressWithIndex: {
            src = Code::deref_adr(ea.size, std::format("{} + {} + {:X}", Code::an(ea.xn), Code::dn(ea.xn), ea.index));
            break;
        }
        ...
        case AddressingMode::PcWithIndex: {
            src = Code::deref_adr(ea.size, std::format("{} + {}", ea.pc_with_index, Code::dn(ea.dst_xn)));
            break;
        }
    }
