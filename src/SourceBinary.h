...
public:
  SourceBinary(std::string path) {
    ...
    if (!file_.is_open()) {
      throw std::invalid_argument(std::format("Cannot open file {}", path));
    }
  }
  ...
  u32 get_next_by_size(Size s) {
    switch (s) {
    case Size::Byte: return get_next_byte();
    case Size::Word: return get_next_word();
    case Size::Long: return get_next_long();
    }
  }
  ...
  u32 get_next_long() {
    ...
    last_read_value_ = (((u32)b[0]) << 24) | (((u32)b[1]) << 16) | (((u32)b[2]) << 8) |
           (((u32)b[3]) << 0);
